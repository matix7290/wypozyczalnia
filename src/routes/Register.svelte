<script>
    ...
    let checkValues = () => {
        ...
        checkRegEx(/^\p{L}+$/u, "firstname", user_info.firstname);
        checkRegEx(/^\p{L}+$/u, "lastname", user_info.lastname);
        checkRegEx(/^\d+$/, "house_number", user_info.house_number);
        checkRegEx(/^\d*$/, "apartment_number", user_info.apartment_number);
        checkRegEx(/^\p{L}+$/u, "city", user_info.city);
        checkRegEx(/^\d{2}-\d{3}$/, "postcode", user_info.postcode);
        checkRegEx(
        ...
        checkRegEx(/^\p{L}+$/u, "owner_firstname", user_info.owner_firstname);
        checkRegEx(/^\p{L}+$/u, "owner_lastname", user_info.owner_lastname);
        checkRegEx(/^\d{16}$/, "card_number", user_info.card_number);
        checkRegEx(
            /^(0[1-9]|1[0-2])\/\d{2}$/,
            "expiry_date",
            user_info.expiry_date
                ...
                <div class="relative mb-4">
                    ...
                    <button
                        ...
                        on:click={() => {
                            errors();
                            checkValues();
                            if (passwords_match && good_values) {
                                register();
                            } else {
                                goTop();
                            }
